...
import { createEnrollmentWithAddress, createUser, createTicketTypeWhitHotel, createTicket, createTicketTypeWhithoutHotel, createHotels, createRooms } from "../factories";
...
describe("GET /hotels", () => {
  ...
  describe("when token is valid", () => {
    ...
    it("should respond with status 402 when user has not paid the ticket", async () => {
      ...
      const ticketType = await createTicketTypeWhitHotel();
    ...
    it("should respond with status 402 when ticket does not include hosting or is remote", async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createTicketTypeWhithoutHotel();
...
describe("GET /hotels/:hotelId", () => {
  ...
  describe("when token is valid", () => {
    ...
    it("should respond with status 200", async () => {
      ...
      const response = await server.get("/hotels/40000").set("Authorization", `Bearer ${token}`);
